...
def data_loader_file(file):
    ...
    id_text = {}
    num = 0
    cancer = {0: '腺癌', 1: '肺良性疾病', 2: '鳞癌', 3: '无法判断组织分型', 4: '复合型', 5: '转移癌', 6: '小细胞癌', 7: '大细胞癌',8: '腺癌+鳞癌', 9: '多形性癌', 10: '大细胞癌+小细胞癌', 11: '大细胞癌+鳞癌', 12: '腺癌+大细胞癌', 13: '小细胞癌+鳞癌', 14: '腺癌+小细胞癌', 15: '鳞癌+梭型细胞癌'}
    t = {0: '无', 1: '转移'}
    l_t = {0: '无', 1: '淋巴转移'}
    ...
    for row in rows:
        ...
        # print(row_val,len(row_val))
        if len(row_val) < 8:
            continue
        while True:
            if row_val[0] in id_text.keys():
                row_val[0] = str(row_val[0]) + "1"
            else:
                break
        # print(row_val)
        id_text[str(row_val[0])] = {"text": "", "list": ['wu', 0, 0, 0, 'wu'], "size": [], "size_index": []}
        id_text[str(row_val[0])]["text"] = row_val[1]
        id_text[str(row_val[0])]["list"][1] = id_cancer[row_val[3]]
        id_text[str(row_val[0])]["list"][2] = id_t[row_val[4]]
        id_text[str(row_val[0])]["list"][3] = id_l_t[row_val[5]]
        if len(row_val) > 9 and row_val[9] != None:
            id_text[str(row_val[0])]["list"][1] = id_cancer[row_val[9]]
        if len(row_val) > 10 and row_val[10] != None:
            id_text[str(row_val[0])]["list"][1] = id_cancer[row_val[10]]
        if row_val[7] == None:
            id_text[str(row_val[0])]["size"] = []
        else:
            aa = str(row_val[7]).split(' ')
            id_text[str(row_val[0])]["size"] = aa
        if row_val[8] == None:
            continue
        k = row_val[8]
        kk = k.split('\t')
        ...
        id_text[str(row_val[0])]["size_index"] = size_index_t
    # print(len(id_text.keys()))
    return id_text
...
def data_loader(data):
    ...
    text_dd = []
    cancar_list = [0 for i in range(16)]
    t_l_list = [0, 0]
    l_t_l_list = [0, 0]
...
class Word_BERT(nn.Module):
    def __init__(self, seq_label=1, cancer_label=16, transfer_label=2, ly_transfer=2):
...
def train_few(train_data, test_data, cancer_id_l, transfer_id_l, lymph_transfer_id_l, text):
    ...
    for epoch in range(epochs):
        for step, batch in enumerate(train_iter):
            ...
            cancer_loss = F.cross_entropy(cancer.view(-1, 16), cancer_label.view(-1))
        ...
        if (epoch + 1) >= 40 and (epoch + 1) % 5 == 0:
            ...
            with open('./result/15/cancer_pred.txt', mode='w', encoding='utf-8') as f:
                for i in range(len(cancer_l[0])):
                    if cancer_l[0][i]!=cancer_l[1][i]:
                        f.writelines(text[i]+'\n')
                        f.writelines(str(cancer_l[0][i])+str(cancer_l[1][i])+'\n')
            ...
            if cancer_result["macro avg"]['f1-score'] > max_f1:
                ...
                torch.save(model.state_dict(), "./result/15/model.pth")
                with open('./result/15/epoch.txt', mode='a', encoding='utf-8') as f:
                    f.writelines(str(epoch)+'\n')
                df = pd.DataFrame(cancer_result).transpose()
                df.to_csv("./result/15/cancer_result.csv", index=True)
                transfer_result = classification_report(transfer_l[1], transfer_l[0], target_names=transfer_id_l,
                                                        output_dict=True)
                df = pd.DataFrame(transfer_result).transpose()
                df.to_csv("./result/15/transfer_result.csv", index=True)
                transfer_l_result = classification_report(ly_transfer_l[1], ly_transfer_l[0],
                                                          target_names=lymph_transfer_id_l, output_dict=True)
                df = pd.DataFrame(transfer_l_result).transpose()
                df.to_csv("./result/15/transfer_l_result.csv", index=True)
                for thresold in [0.05, 0.08, 0.1, 0.15, 0.2, 0.3, 0.4]:
                    ...
                    with open('./result/15/size.txt', mode='a', encoding='utf-8') as f:
                        f.writelines("阈值" + str(thresold) + '\n')
                        f.writelines("F1:" + str(round(f1 * 100, 2)) + "pre" + str(round(p * 100, 2)) + "recall" + str(
                            round(r * 100, 2)) + "\n")
            model.train()
...
if __name__ == '__main__':
    ...
    # train_new = {}
    # for i,j in train_text.items():
    #     # print(i,j)
    #     if train_text[i]["list"][1] == 0 and train_text[i]["list"][3] == 0 and train_text[i]["list"][2] == 0:
    #         if random.randint(0,3)!=0:
    #             continue
    #     if train_text[i]["list"][1] == 4:
    #         train_new[i+"_1"] = train_text[i]
    #     if train_text[i]["list"][1] in [5,6]:
    #         for k in ["_1","_2","_3","_4","_5","_6","_7","_8"]:
    #             train_new[i+k] = train_text[i]
    #     if train_text[i]["list"][1] == 7:
    #         for k in ["_1","_2","_3","_4","_5","_6"]:
    #             train_new[i+k] = train_text[i]
    #     if train_text[i]["list"][2] == 1:
    #         for k in range(20):
    #             train_new[i+str(k)] = train_text[i]
    #     if train_text[i]["list"][3] == 1:
    #         for k in range(5):
    #             train_new[i+str(k)] = train_text[i]
    #     train_new[i] = train_text[i]

    test_text = data_loader_file('./测试集_排错.xlsx')

    human = data_loader_file('./数据_造_排错.xlsx')
    # print(human)
    ...
    #     print(i,j)
    for i, j in human.items():
        if i in train_text.keys():
            # print(i,j)
            train_text[i + str(1)] = j
        else:
            train_text[i] = j
    print(len(train_text.keys()))
    ...
    #     cancer_nums[train_text[i]["list"][1]] += 1
    #     transfer_nums[train_text[i]["list"][2]] += 1
    #     lymph_transfer_nums[train_text[i]["list"][3]] += 1
    # print(cancer_nums,transfer_nums,lymph_transfer_nums)
    # cancer_nums = {'腺癌': 0, '肺良性疾病': 0, '鳞癌': 0, '无法判断组织分型': 0, '复合型': 0, '转移癌': 0, '小细胞癌': 0, '大细胞癌': 0}
    # transfer_nums = {'无': 0, '转移': 0}
    # lymph_transfer_nums = {'无': 0, '淋巴转移':1}
    # for i,j in test_text.items():
    #     # print(raw_mark_ali[i]["list"])
    #     cancer_nums[test_text[i]["list"][1]] += 1
    #     transfer_nums[test_text[i]["list"][2]] += 1
    #     lymph_transfer_nums[test_text[i]["list"][3]] += 1
    # print(cancer_nums,transfer_nums,lymph_transfer_nums)
