    ...
    for epoch in range(epochs):
        ...
        # if (epoch + 1) == epochs:
            ...
            if cancer_result["macro avg"]['f1-score']>max_f1:
                ...
                torch.save(model.state_dict(), "./result/13/model.pth")
                with open('./result/13/epoch.txt', mode='a', encoding='utf-8') as f:
                    f.writelines(str(epoch)+'\n')
                df = pd.DataFrame(cancer_result).transpose()
                df.to_csv("./result/13/cancer_result.csv", index= True)
                ...
                df.to_csv("./result/13/transfer_result.csv", index= True)
                ...
                df.to_csv("./result/13/transfer_l_result.csv", index= True)
                for thresold in [0.05,0.08,0.1,0.15,0.2,0.3,0.4]:
                    ...
                    with open('./result/13/size.txt', mode='a', encoding='utf-8') as f:
...
if __name__=='__main__':
    ...
    train_data,text = data_loader(train_new)
