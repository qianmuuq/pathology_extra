...
def train_few(train_data, test_data,cancer_id_l,transfer_id_l,lymph_transfer_id_l,text):
    ...
    for epoch in range(epochs):
        ...
        # if (epoch + 1) == epochs:
            ...
            #         print(text[i])
            #         print(cancer_l[0][i],cancer_l[1][i])
            #         print()
            ...
            # print(cancer_result)
            if cancer_result["weighted avg"]['f1-score']>max_f1:
                max_f1 = cancer_result["weighted avg"]['f1-score']
                torch.save(model.state_dict(), "./result/14/model.pth")
                with open('./result/14/epoch.txt', mode='a', encoding='utf-8') as f:
                    f.writelines(str(epoch)+'\n')
                df = pd.DataFrame(cancer_result).transpose()
                df.to_csv("./result/14/cancer_result.csv", index= True)
                ...
                df.to_csv("./result/14/transfer_result.csv", index= True)
                ...
                df.to_csv("./result/14/transfer_l_result.csv", index= True)
                for thresold in [0.05,0.08,0.1,0.15,0.2,0.3,0.4]:
                    ...
                    with open('./result/14/size.txt', mode='a', encoding='utf-8') as f:
