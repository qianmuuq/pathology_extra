...
class Word_BERT(nn.Module):
    def __init__(self, seq_label=1,cancer_label=8,transfer_label=2,ly_transfer=2):
        ...
        self.bert = BertModel.from_pretrained('F:\\transformers_\\bert-base-zh')
    ...
    def forward(self, word_input, masks):
        ...
        return out,cancer,transfer,ly_transfer,pool
...
def train_few(train_data, test_data,cancer_id_l,transfer_id_l,lymph_transfer_id_l,text):
    ...
    for epoch in range(epochs):
        for step, batch in enumerate(train_iter):
            ...
            text_input, seq_out, mask_input, cancer_label, t_l, l_t_l = text_input.cuda(), seq_out.cuda(), mask_input.cuda(), cancer_label.cuda(), t_l.cuda(), l_t_l.cuda()
            out,cancer,transfer,ly_transfer,pool = model(text_input, mask_input)
            ...
            # print(pool.size())
            # print()
            _,_,_,_,pool_2 = model(text_input, mask_input)
            hidden_states = torch.cat([pool,pool_2]).unsqueeze(1)
            # print(hidden_states.size())
            # print(cancer_label.size())
            labels_t = cancer_label.repeat(2)
            hidden_states = F.normalize(hidden_states, dim=-1)
            loss_cir = cir(features=hidden_states, labels=labels_t)
            loss += 0.1 * loss_cir
            ...
            if (step+1)%6==0 or (step+1)==len(train_iter):
                optimizer.step()
                scheduler.step()
                model.zero_grad()
        ...
        # if (epoch + 1) == epochs:
            ...
            with torch.no_grad():
                ...
                for batch in test_iter:
                    text_input, seq_out, mask_input, cancer_label, t_l, l_t_l = batch
                    text_input, seq_out, mask_input, cancer_label, t_l, l_t_l = text_input.cuda(), seq_out.cuda(), mask_input.cuda(), cancer_label.cuda(), t_l.cuda(), l_t_l.cuda()
                    out,cancer,transfer,ly_transfer,_ = model(text_input, mask_input)
            ...
            #         f.writelines('\n')
            print(classification_report(cancer_l[1], cancer_l[0], target_names=cancer_id_l))
            print(classification_report(transfer_l[1], transfer_l[0], target_names=transfer_id_l))
            print(classification_report(ly_transfer_l[1], ly_transfer_l[0], target_names=lymph_transfer_id_l))
            model.train()
...
if __name__=='__main__':
    train_text = data_loader_file('./训练集.xlsx')
    # train_new = {}
    ...
    test_text = data_loader_file('./测试集.xlsx')

    human = data_loader_file('./数据_造.xlsx')
    ...
    # lymph_transfer_nums = {'无': 0, '淋巴转移':0}
    # for i,j in train_text.items():
    ...
    # print(cancer_nums,transfer_nums,lymph_transfer_nums)
    tokenizer = AutoTokenizer.from_pretrained('F:\\transformers_\\bert-base-zh', use_fast=True)

    config = AutoConfig.from_pretrained('F:\\transformers_\\bert-base-zh')
